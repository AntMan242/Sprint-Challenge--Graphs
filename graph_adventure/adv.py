...
#Do a BFS to take player back to nearest room thay has an unexplored exit
def unexplored(current_path, room_id, player):
    visited = set()
    #Create a queue and enqueue a starting path as a vertex
    qq = Queue()
    path = [room_id]
    qq.enqueue(path)
    ...
    while qq.size() > 0:
        ...
        p = qq.dequeue()
        #set vertex ->(v) as the first path
        v = p[0]
        #If v(vertex) is '?' set the path and breakout of the if
        if v == '?':
            path = p[1:]
            break
        #If vertex v is not visited, set it as visited
        if v not in visited:
            visited.add(v)
            #Add a path to all of its neighbors and add it to queue
            for neighbors in current_path[v]:
                node = current_path[v][neighbors]
                new_path = p.copy()
                new_path.insert(0, node)
                qq.enqueue(new_path)
    #Set player_move_directions list to contain directions a player needs to go back
    player_move_directions = []
    #create a path of player_move_directions while the loop runs
    while len(path) > 1:
        #Remove the last room and store it as a variable called 'current' 
        current = path.pop(-1)
        #Find prior directions that will take you back to previous room and add it to player_move_directions list
        for route in current_path[current]:
            if current_path[current][route] == path[-1]:
                player_move_directions.append(route)
    #For every element in player_move_directions, move in those directions until you reach the spot
...
#Function to get the opposite direction to update our '?' unexplored exits for the room we came from
...
while len(visited) < len(roomGraph):
    ...
    if room_id not in visited:
        visited[room_id] = { ways: '?' for ways in room_exits }
    #Get current room exits
    unvisited = [way for way in visited[room_id] if visited[room_id][way] == '?']
    #If unvisited is not an empty list, explore an unexplored room
    if len(unvisited) > 0:
        ...
        #Add the travel destination to the main path
        traversalPath.append(next_way)
        #Next room is now known, so set it for previous room's direction index
        next_room_Id = player.currentRoom.id
        visited[room_id][next_way] = next_room_Id
        #If the next room is not in visited, lets add it
        if next_room_Id not in visited:
            next_room_exits = player.currentRoom.getExits()
            visited[next_room_Id] = { ways: '?' for ways in next_room_exits }
        #The previous room we were in is now the opposite direction of the current room
        opposite_way = opposite_direction(next_way)
        visited[next_room_Id][opposite_way] = room_id
    #Else find a way back to the nearest unexplored room using breadth first search (BFS)
    else:
        traversalPath += unexplored(visited, room_id, player)
